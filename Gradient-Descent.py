...
def encode_data(df):
    ...
    """
    ### BEGIN SOLUTION
    name_u, array_u, num_users = proc_col(df['userId']) 
    name_m, array_m, num_movies = proc_col(df['movieId'])
    # ! There was no need to create a whole new dataframe, 
    # ! you could have just replaced the old 'uderId' of 'movieId' columns with the new ones 
    df = pd.DataFrame({'userId':array_u, 'movieId':array_m, 'rating':df['rating']})
    ### END SOLUTION
...
def encode_new_data(df_val, df_train):
    ...
    """
    ### BEGIN SOLUTION
    # I NEED TO FIND UNIQUE VALUES IN DF_TRAIN AND ONLY ALLOW THOSE VALUES IN DF_VAL 
    uniq_users = df_train['userId'].unique()
    df_val = df_val[df_val['userId'].isin(uniq_users)]
    uniq_movies = df_train['movieId'].unique()
    df_val = df_val[df_val['movieId'].isin(uniq_movies)]
# ? check to see if this code is inferior to the anser code? 
    name_u, array_u, num_users = proc_col(df_val['userId'])
    name_m, array_m, num_movies = proc_col(df_val['movieId'])
    df_val = pd.DataFrame({'userId':array_u, 'movieId':array_m, 'rating':df_val['rating']})
    ### END SOLUTION
    return df_val
...
def cost(df, emb_user, emb_movie):
    ...
    """
    ### BEGIN SOLUTION
    prediction = emb_user[i]*emb_movie[j] 
    error = df['rating'] - prediction
